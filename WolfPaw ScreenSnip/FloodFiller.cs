	...
	public class FloodFiller : AbstractFloodFiller {
		...
		public override void FloodFill(Bitmap bmp, Point pt)
		{
		    ...
		    unsafe
		    {
				...
				long loc =CoordsToIndex(pt.X,pt.Y,bmpData.Stride);//((bmpData.Stride*(pt.Y-1))+(pt.X*4));
		...
		unsafe void LinearFloodFill4( byte* scan0, int x, int y,Size bmpsize, int stride, byte* startcolor)
		{
			...
			long* p=(long*) (scan0+(CoordsToIndex(x,y, stride)));
			
			
			//FIND LEFT EDGE OF COLOR AREA
			int LFillLoc=x; //the location to check/fill on the left
			long* ptr=p; //the pointer to the current location
			while(true)
			{
			...
			ptr=(long*)(scan0+CoordsToIndex(LFillLoc,y,stride));
		...
		long CoordsToIndex(long x, long y, long stride)
		{
			return (stride*y)+(x*4);
		}
